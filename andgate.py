...
class AndGate(GUIComponent):
  ...
  def __init__(self, window, canvas, component_id, x, y):  # pylint: disable=too-many-arguments, too-many-positional-arguments
    ...
    canvas.tag_bind(self.component_shapes[0], "<ButtonPress-1>",
                    self.start_drag)
    canvas.tag_bind(self.component_shapes[0], "<B1-Motion>", self.drag)
    canvas.tag_bind(self.component_shapes[0], "<ButtonRelease-1>",
                    self.stop_drag)

    canvas.tag_bind(self.component_shapes[-1], "<ButtonPress-1>",
                    self.start_drag)
    canvas.tag_bind(self.component_shapes[-1], "<B1-Motion>", self.drag)
    canvas.tag_bind(self.component_shapes[-1], "<ButtonRelease-1>",
                    self.stop_drag)

    canvas.tag_bind(f"AND_{self.component_id}_CLICKABLE", "<Enter>",
                    self.handle_hover_enter)
    canvas.tag_bind(f"AND_{self.component_id}_CLICKABLE", "<Leave>",
